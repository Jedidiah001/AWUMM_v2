...
class StatsTracker:
    ...
    def get_wrestler_stats(self, wrestler_id: str) -> Optional[WrestlerStats]:
        ...
        stats.total_main_events = achievements['main_events']
        stats.total_ppv_matches = achievements['ppv_matches']
        stats.total_upsets = achievements['upsets_caused']
        stats.total_upset_losses = achievements['upset_losses']
    ...
    def check_for_milestones(self, wrestler_id: str, stats: WrestlerStats, show_result: ShowResult):
        ...
        # Check main event milestones (FIXED)
        if stats.total_main_events == 50:
            milestones_achieved.append((
                MilestoneType.MAIN_EVENT_50,
                f"{stats.wrestler_name} has main evented 50 shows!"
            ))
        
        # Check PPV milestones (FIXED)
        if stats.total_ppv_matches == 50:
            milestones_achieved.append((
